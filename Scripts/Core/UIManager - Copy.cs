...
namespace SpaceEngineersShipBuilder
{
    public class UIManager
    {
        ...
        public void ToggleItemPanel()
        {
            ...
            {
                CreateUI(); // Reset to the main UI
                _isItemPanelVisible = false;
            }
            else
            {
                _itemPanel.Itempanel();  // Ensure the ItemPanel is created and its layout is set
                _desktop.Root = _itemPanel.GetRootWidget();  // Assuming GetRootWidget returns the root widget of the ItemPanel
                _isItemPanelVisible = true;
            }
        }
        ...
        public bool IsItemPanelVisible => _isItemPanelVisible;
    }
}
